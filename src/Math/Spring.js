...
export function rungeKuttaArray(dt, position, velocity, frequency, damping) {
    ...
    const k1x = velocity;
    const k1v = sub(mul_s(velocity, d * w), mul_s(position, w2));

    const k2x = add(velocity, mul_s(k1v, dt2));
    const k2v = sub(mul_s(add(velocity, mul_s(k1v, dt2)), d * w), mul_s(add(position, mul_s(k1x, dt2)), w2));

    const k3x = add(velocity, mul_s(k2v, dt2));
    const k3v = sub(mul_s(add(velocity, mul_s(k2v, dt2)), d * w), mul_s(add(position, mul_s(k2x, dt2)), w2));

    const k4x = add(velocity, mul_s(k3v, dt));
    const k4v = sub(mul_s(add(velocity, mul_s(k3v, dt)), d * w), mul_s(add(position, mul_s(k3x, dt)), w2));

    return [
        add(position, mul_s(add(add(k1x, mul_s(k2x, 2)), add(mul_s(k3x, 2), k4x)), dt6)),
        add(velocity, mul_s(add(add(k1v, mul_s(k2v, 2)), add(mul_s(k3v, 2), k4v)), dt6)),
    ];
}
...
export function updateAllSprings(dt) {
    ...
    for (const spring of handledSprings) {
        ...
        const position = isNumber ? spring.position - g : sub(spring.position, g)
        ...
        spring.position = isNumber ? newPosition + g : add(/**@type {number[]}*/(newPosition), g);
