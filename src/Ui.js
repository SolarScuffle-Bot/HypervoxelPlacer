...
/**
 * Convert HSL (with Alpha) in [0,1] to RGBA in [0,1].
 * @param {HSLA} hsla - Normalized HSLA color
 * @returns {RGBA} RGBA in [0,1]
 */
function hsla_to_rgba(hsla) {
	...
	// 2) Calculate intermediate X based on fractional Hue
	const hprime = 6 * h; // Range [0..6)
	const x = c * (1 - Math.abs((hprime % 2) - 1))

	// 3) Determine R', G', B' based on the interval H' is in
	...
	// 4) Add the match component M to shift to correct lightness
	const m = l - c / 2
	const r = rprime + m
	const g = gprime + m
	const b = bprime + m

	// 5) The Alpha is unchanged
	return [r, g, b, a]
  }

const hueSteps = 15
const hueScale = (hueSteps - 1) / hueSteps
export let sliderHue = 0
const hue_slider = new_slider({
	minX: -Canvas.uw * 45,
	minY: Canvas.uh * 20,
	maxX: -Canvas.uw * 45,
	maxY: Canvas.uh * -20,
	...
	stroke: 2,
	steps: hueSteps,
	color: a => { return hsla_to_rgba([hueScale * a, 0.5, 0.5, 1]) }
})

export let sliderSaturation = 0
const saturation_slider = new_slider({
	minX: -Canvas.uw * 42.5,
	minY: Canvas.uh * 20,
	maxX: -Canvas.uw * 42.5,
	maxY: Canvas.uh * -20,
	...
	stroke: 2,
	steps: 6,
	color: a => { return hsla_to_rgba([sliderHue, a, 0.5, 1]) }
})

export let sliderLightness = 0
const lightness_slider = new_slider({
	minX: -Canvas.uw * 40,
	minY: Canvas.uh * 20,
	maxX: -Canvas.uw * 40,
	maxY: Canvas.uh * -20,
	...
	stroke: 2,
	steps: 8,
	color: a => { return hsla_to_rgba([sliderHue, sliderSaturation, a, 1]) }
})

const alphaScale = 0.95 // We don't want people making truly transparent cells now
export let sliderAlpha = 0
const alpha_slider = new_slider({
	minX: -Canvas.uw * 37.5,
	minY: Canvas.uh * 20,
	maxX: -Canvas.uw * 37.5,
	maxY: Canvas.uh * -20,
	...
	stroke: 2,
	steps: 6,
	color: a => { return hsla_to_rgba([sliderHue, sliderSaturation, sliderLightness, 1 - alphaScale * a]) }
})
...
export function renderAll() {
	...
	// sliderSaturation = saturationAlpha

	// const lightnessAlpha = get_slider_color_alpha(lightness_slider)
	// sliderLightness = lightnessAlpha

	// const alphaAlpha = get_slider_color_alpha(alpha_slider)
	// sliderAlpha = alphaScale * alphaAlpha
